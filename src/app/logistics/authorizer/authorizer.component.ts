...
import { SettingAuthorizer } from './interfaces/setting-authorizer.interface';
import { ModalUserComponent } from '../../security/shared/components/modal-user/modal-user.component';
...
export class AuthorizerComponent {
  ...
  public setting: SettingAuthorizer;
  public table: Table;
  ...
  public save() {
    if (this.model.valid) {
      let object: Authorizer = Object.assign({}, this.model.value)
  ...
  public searchUser() {
    this.bsModalRef = this.bsModalService.show(ModalUserComponent, { class: 'modal-lg modal-dialog-centered', backdrop: 'static'})
    this.bsModalRef.content.response.subscribe((response: User) => {
      if (response != null) {
        this.model.get('idUser')?.setValue(response.id);
        this.model.get('userName')?.setValue(response.name);
  ...
  public cleanUser() {
    this.model.get('idUser')?.setValue('');
    this.model.get('userName')?.setValue('');
  }
