...
@Component({
  selector: 'app-modal-purchase-order',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgxSpinnerModule, FormsModule],
  templateUrl: './modal-purchase-order.component.html'
})
export class ModalPurchaseOrderComponent {
  @ViewChild('filterId') filterId!: ElementRef;
  @Output() response = new EventEmitter();

  public lstFilter: Filter[];
  public spinnerName: string;
  public setting:  { recordId: number, filterSupplier: string };
  ...
  constructor(
    ...
    this.setting = { recordId: 0, filterSupplier: '' };
    this.value = { id: 0 }
    ...
    this.paged = this.formBuilder.group({
      ...
      lstFilter: this.formBuilder.array(
        [
          this.formBuilder.group({ object: 'purchaseOrder', column: 'id', value: '', operator: 'equal' }),
    ...
    this.table = {
      ...
      lstColumn: [
        ...
        { name: 'PROVEEDOR', width: '38%', style: 'text-start' },
  ...
  ngOnInit(): void {
    ...
    this.spinner.show()
    this.service.settingDetail(object).subscribe({
      next: (response) => {
  ...
  public search() {
    if (this.paged.valid) {
      ...
      this.service.findByPaginationDetail(object).subscribe({
        next: (response) => {
          if (response !== null && response.status == 'ok') {
