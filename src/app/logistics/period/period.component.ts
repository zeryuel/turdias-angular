...
import { SettingPeriod } from './interfaces/setting-period.inetrface';
...
export class PeriodComponent {
  ...
  constructor(
    ...
    this.model = this.formBuilder.group({
      id: '',
      year: [parseInt(moment().format('YYYY')), Validators.required],
      closeDate: '',
      processDate: '',
      month: this.formBuilder.group({
        id: [parseInt(moment().format('MM')), Validators.required],
        name : ''
      }),
    ...
    this.paged = this.formBuilder.group({
      pageSize: 10,
      pageNumber: 1,
      orderColumn: 'period.id',
      order: 'ASC',
      lstFilter: this.formBuilder.array(
        [
          this.formBuilder.group({ object: 'period', column: 'year', value: '', operator: 'equal' }),
    ...
    this.table = {
      ...
      lstColumn: [
        ...
        { name: 'AÑO', width: '10%', style: 'text-left' },
        { name: 'MES', width: '20%', style: 'text-left' },
        { name: 'FECHA CIERRE', width: '20%', style: 'text-center' },
        { name: 'FECHA PROCESAMIENTO', width: '20%', style: 'text-center' },
    ...
    this.service.setting(object).subscribe({
      next: (response) => {
        if (response !== null && response.status == 'ok') {
          ...
          this.setting.lstMonth = response.value.lstMonth.value;
          this.setting.lstState = response.value.lstState.value;

          this.filterYear.nativeElement.focus();
          this.spinner.hide()
        } else
  ...
  public processInitialBalance() {
    // let initialState = { tipo: 3, opcion: 6 };
    // this.bsModalRef = this.bsModalService.show(ModalMessageComponent, { initialState, backdrop: 'static', class: 'modal-dialog-custom' });
    // this.bsModalRef.content.response.subscribe((value: any) => {
    ...
    //     this.spinner.show()
    //     this.saldoMensualservice.procesar(objeto).subscribe({
    //       next: (response) => {
    //         if (response !== null && response.status == 'ok') {
    //           this.setting.recordId = 0;
  ...
  public save() {
    if (this.model.valid) {
      let object: Period = Object.assign({}, this.model.value)
  ...
  public exit(updatedRecord?: boolean) {
    this.model.reset({
      ...
      closeDate: '',
      processDate: '',
      month: { id: parseInt(moment().format('MM')), name : '' },
      state: { id: 61, name : '' }
    })
