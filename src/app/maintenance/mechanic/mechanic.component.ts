...
import { Mechanic } from './interfaces/mechanic.interface';
import { SettingMechanic } from './interfaces/setting-mechanic.interface';
...
export class MechanicComponent {
  ...
  public setting: SettingMechanic;
  public table: Table;
  ...
  constructor(
    ...
    this.paged = this.formBuilder.group({
      pageSize: 10,
      pageNumber: 1,
      orderColumn: 'mechanic.id',
      order: 'ASC',
      lstFilter: this.formBuilder.array(
        [
          this.formBuilder.group({ object: 'personal', column: 'name', value: ['', Validators.pattern(/^[a-zA-Z0-9 ]+$/)], operator: 'like' }),
          this.formBuilder.group({ object: 'mechanic', column: 'active', value: 1, operator: 'equal'})
        ]
      )
    });
    ...
    this.table = {
      ...
      lstColumn: [
        ...
        { name: 'N° DOCUMENTO', width: '12%', style: 'text-center' },
        { name: 'NOMBRES Y APELLIDOS', width: '63%', style: 'text-left' },
  ...
  public save() {
    if (this.model.valid) {
      let object: Mechanic = Object.assign({}, this.model.value);
      ...
      if (object.id == 0) {
        ...
        this.service.insert(object).subscribe({
          next: (response) => {
            if (response !== null && response.status == 'ok') {
              this.model.get('id')?.setValue(this.pad(response.value, 3, 0));
  ...
  public searchPersonal() {
    ...
    this.bsModalRef.content.response.subscribe((response: Personal) => {
      if (response != null) {
        ...
        this.model.get('documentNumber')?.setValue(response.documentNumber);
        this.model.get('fullName')?.setValue(response.fullName);
      }
    })
  }
  ...
  public cleanPersonal() {
    ...
    this.model.get('documentNumber')?.setValue('');
    this.model.get('fullName')?.setValue('');
  }
