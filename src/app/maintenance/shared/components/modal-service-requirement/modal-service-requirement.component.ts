...
import { ServiceRequirementService } from './../../../service-requirement/services/service-requirement.service';
...
@Component({
  selector: 'app-modal-service-requirement',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgxSpinnerModule, FormsModule],
  templateUrl: './modal-service-requirement.component.html'
})
export class ModalServiceRequirementComponent {
  @ViewChild('filterId') filterId!: ElementRef;
  @Output() response = new EventEmitter();

  public lstFilter: Filter[];
  public spinnerName: string;
  public setting: { recordId: number, filterVehicle: string };
  public value: { id: number };
  ...
  constructor(
    ...
    this.setting = { recordId: 0, filterVehicle: '' };
    this.value = { id: 0 }
    ...
    this.paged = this.formBuilder.group({
      ...
      lstFilter: this.formBuilder.array(
        [
          this.formBuilder.group({ object: 'serviceRequirement', column: 'id', value: '', operator: 'equal' }),
    ...
    this.table = {
      ...
      lstColumn: [
        ...
        { name: 'VEHICULO', width: '76%', style: 'text-start' },
        { name: 'FECHA', width: '10%', style: 'text-center' }
      ],
      lstPageSize: [
        { id: 10, name: '10' },
        { id: 30, name: '30' },
