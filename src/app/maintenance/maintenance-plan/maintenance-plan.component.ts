...
import { SettingMaintenancePlan } from './interfaces/setting-maintenance-plan.interface';
...
export class MaintenancePlanComponent {
  ...
  public setting: SettingMaintenancePlan;
  public table: Table;
  ...
  constructor(
    ...
    this.paged = this.formBuilder.group({
      pageSize: 10,
      pageNumber: 1,
      orderColumn: 'maintenancePlan.id',
      order: 'ASC',
      lstFilter: this.formBuilder.array(
        [
          this.formBuilder.group({ object: 'maintenancePlan', column: 'name', value: ['', Validators.pattern(/^[a-zA-Z0-9 ]+$/)], operator: 'like' }),
          this.formBuilder.group({ object: 'maintenancePlan', column: 'active', value: 1, operator: 'equal'})
        ]
      )
    });
    ...
    this.table = {
      ...
      lstColumn: [
        ...
        { name: 'NOMBRE', width: '50%', style: 'text-left' },
        { name: 'VEHICULO', width: '25%', style: 'text-left' },
  ...
  public save() {
    if (this.model.valid) {
      let object: any = Object.assign({}, this.model.value);
